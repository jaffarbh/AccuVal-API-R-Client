...
apikey <- "b1cf310b-e461-4eec-bce7-4dc23fb7d909"
...
Valuations <- foreach(i = 1:nrow(Properties) , .combine = 'rbind', .packages=c('foreach', 'data.table', 'dplyr', 'httr', 'jsonify', 'stringr')) %dopar% {
  ...
  new <- as.character(Properties[i,"new"])
  tenure <- as.character(Properties[i,"tenure"])
  condition <- as.character(Properties[i,"condition"])
  epc <- as.character(Properties[i,"epc"])
  age <- as.character(Properties[i,"age"])
  uuid <- as.character(Properties[i,"uuid"])
  
  # Paste together
  BODY <- paste0("?", "postcode=", postcode, "&type=", type, "&rooms=", rooms, "&area=", area,
                 "&new=", new, "&tenure=", tenure, "&condition=", condition,
                 "&epc=", epc, "&age=", age)
